...
func download(release *github.RepositoryRelease, name string) ([]byte, error) {
...
func release(source string, destination string) error {
	...
	binary, err := download(sourceRelease, "Country.mmdb")
	if err != nil {
		return err
	}
	metadata, countryMap, err := parse(binary)
...
func main() {
	...
	if len(os.Args) >= 3 {
		...
		err = release("Loyalsoldier/geoip", "ShinyNito/sing-geoip")
	}
	if err != nil {
		logrus.Fatal(err)
	}
